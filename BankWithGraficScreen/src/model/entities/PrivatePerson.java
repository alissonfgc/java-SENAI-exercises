...
/**
 * Class PrivatePerson for validate the CPF on Main class
 * 
 * @author alissonfgc
 */
...
public class PrivatePerson {
	...
	public PrivatePerson(String cpf) {
		this.cpf = cpf;
		if (cpf.length() == 11) {
			this.cpf = cpf.substring(0, 3) + "." + (cpf.substring(3, 6)) + "." + (cpf.substring(6, 9)) + "-"
					+ (cpf.substring(9, 11));
		}
		String s1, s2, s3, s4, s5, s6, s7, s8, s9;
		int n1, n2, n3, n4, n5, n6, n7, n8, n9, tester1, tester2;

		s1 = this.cpf.substring(0, 1);
		n1 = Integer.parseInt(s1);
		s2 = this.cpf.substring(1, 2);
		n2 = Integer.parseInt(s2);
		s3 = this.cpf.substring(2, 3);
		n3 = Integer.parseInt(s3);
		s4 = this.cpf.substring(4, 5);
		n4 = Integer.parseInt(s4);
		s5 = this.cpf.substring(5, 6);
		n5 = Integer.parseInt(s5);
		s6 = this.cpf.substring(6, 7);
		n6 = Integer.parseInt(s6);
		s7 = this.cpf.substring(8, 9);
		n7 = Integer.parseInt(s7);
		s8 = this.cpf.substring(9, 10);
		n8 = Integer.parseInt(s8);
		s9 = this.cpf.substring(10, 11);
